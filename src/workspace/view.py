...
from qtpy import QtCore, QtWidgets, QtGui, Signal, Slot, QtSql

# Local application/library specific imports.
from workspace.model import WorkspaceModel
...
class StateDelegate(QtWidgets.QStyledItemDelegate):
    ...
    def initStyleOption(self, option: QtWidgets.QStyleOptionViewItem, index: QtCore.QModelIndex) -> None:
        ...
        if value != "":
            ...
            option.decorationAlignment = QtCore.Qt.AlignmentFlag.AlignHCenter | QtCore.Qt.AlignmentFlag.AlignCenter
            option.displayAlignment = QtCore.Qt.AlignmentFlag.AlignHCenter | QtCore.Qt.AlignmentFlag.AlignCenter
            option.text = ""
...
class WorkspaceManagerDialog(QtWidgets.QDialog):
    ...
    def initUI(self):
        ...
        self.edit_button.setIcon(theme_icon_manager.get_icon(':pencil'))
        ...
        self.workspace_table = QtWidgets.QTableView()
        self.workspace_table.setModel(self._model)
        self.workspace_table.horizontalHeader().setStretchLastSection(True)
        self.workspace_table.setEditTriggers(QtWidgets.QAbstractItemView.EditTrigger.NoEditTriggers) # ReadOnly
        ...
        for field in WorkspaceModel.Fields.fields():
            if not field.visible:
                self.workspace_table.hideColumn(field.index)

        state_delegate = StateDelegate()
        self.workspace_table.setItemDelegateForColumn(WorkspaceModel.Fields.State.index,
                                                      state_delegate)
