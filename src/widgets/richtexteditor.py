...
class RichTextEditor(QtWidgets.QWidget):
    ...
    @saveMode.setter
    def saveMode(self, mode: SaveMode):
        self._saveMode = mode
    ...
    @classmethod
    def fromFile(cls, path, parent=None):
        try:
            text = cls.loadFile(path)
        except Exception as e:
            return
        else:
            saveMode = RichTextEditor.SaveMode.FileMode
            uid = queryFileID(path)
            return cls(path, text, saveMode, uid, parent)
        
    @classmethod
    def fromMapper(cls, bar, parent=None):
        saveMode = RichTextEditor.SaveMode.MapperMode
        return cls(saveMode=saveMode, bar=bar, parent=parent)
    
    @classmethod
    def loadFile(cls, path: str) -> str:
        fh = None
        try:
            ...
            if not fh.open(QtCore.QIODevice.OpenModeFlag.ReadOnly):
                ...
                QtWidgets.QMessageBox.warning(cls,
                                              ...
                                              f"Failed to open {path}: {IOError(fh.errorString())}")
                return IOError
    ...
    def connectSignals(self):
        ...
        if self.saveMode == RichTextEditor.SaveMode.FileMode:
            self.editor.textChanged.connect(self.saveFile)
    ...
    def createToolbar(self):
        ...
        self.toolbar.addAction(self.strikeout_action)
